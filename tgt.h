...
static void lightpblk_print_geo(struct nvm_geo *geo){
	...
	pr_notice("geo->major_ver_id = %d,geo->minor_ver_id = %d,geo->version = %d,geo->num_ch = %d,geo->num_lun = %d, \
		geo->all_luns = %d,geo->all_chunks = %d,geo->op = %d,geo->total_secs = %d, geo->num_chk = %d,geo->clba = %d, \
		geo->csecs = %d,geo->sos = %d,geo->ws_min = %d,geo->ws_opt = %d,geo->mw_cunits = %d,geo->maxoc = %d, \
		geo->maxocpu = %d\n",geo->major_ver_id,geo->minor_ver_id,geo->version,geo->num_ch,geo->num_lun,geo->all_luns,geo->all_chunks,
		geo->op,geo->total_secs, geo->num_chk,geo->clba,geo->csecs,geo->sos,geo->ws_min,geo->ws_opt,geo->mw_cunits,
		geo->maxoc,geo->maxocpu);

	pr_notice("geo->mccap = %d, geo->trdt = %d, geo->trdm = %d,geo->tprt = %d,geo->tprm = %d,geo->tbet = %d,geo->tbem = %d\n",
		geo->mccap, geo->trdt, geo->trdm,geo->tprt,geo->tprm,geo->tbet,geo->tbem);

	pr_notice("geo->addrf.ch_len = %d,geo->addrf.lun_len = %d,geo->addrf.chk_len = %d,geo->addrf.sec_len = %d,geo->addrf.rsv_len[0] = %d, \
		geo->addrf.ch_offset = %d,geo->addrf.lun_offset = %d,geo->addrf.chk_offset = %d,geo->addrf.sec_offset = %d, \
		geo->addrf.rsv_off[0] = %d,geo->addrf.ch_mask = 0x%llx,geo->addrf.lun_mask = 0x%llx,geo->addrf.chk_mask = 0x%llx, \
...
//	dst->blk_offset = dst->pg_offset + dst->pg_len;
	...
	dst->ch_offset = srcppaf->ch_offset;
	dst->lun_offset = srcppaf->lun_offset;
	dst->pg_offset = srcppaf->pg_offset;
	dst->blk_offset = srcppaf->blk_offset;
...
//pr_notice("Enter lightpblk_init()\n");
	...
	alightpblk->paralun = geo->num_ch * geo->num_lun;
	alightpblk->nrpl = geo->num_pln;
	alightpblk->nrlun = geo->all_luns;
	alightpblk->sec_per_pg = geo->clba;
...
static int tgt_setup_w_rq(struct f2fs_sb_info* sbi, struct nvm_rq* rqd, int nr_secs, nvm_end_io_fn(*end_io)){
	...
	rqd->nr_ppas = nr_secs;
	rqd->flags = (geo->pln_mode >> 1) | NVM_IO_SCRAMBLE_ENABLE;
	rqd->private = sbi;
	rqd->end_io = end_io;
